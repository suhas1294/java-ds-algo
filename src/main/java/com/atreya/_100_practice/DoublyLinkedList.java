...
public class DoublyLinkedList {
    ...
    @AllArgsConstructor
    private class Searcher implements Callable<Integer> {
        ...
        @Override
        public Integer call() {
            System.out.println("I am executed parally\t" + this.direction);
            if (this.direction.equalsIgnoreCase("forward")){
                Node current = list.head;
                for (int i = 0; i < list.size/2; i++) {
                    if (current.value == targetNUmber)return i;
                    current = current.next;
                }
            }else{
    ...
    public int search(int targetElement) throws ExecutionException, InterruptedException {
        ...
        Searcher forwardSearcher = new Searcher(targetElement, this, "forward");
        Searcher reverseSearcher = new Searcher(targetElement, this, "backward");
        Future<Integer> forwardResult =  service.submit(forwardSearcher);
        Future<Integer> backwardResult =  service.submit(reverseSearcher);
    ...
    public static DoublyLinkedList listFactory(){
